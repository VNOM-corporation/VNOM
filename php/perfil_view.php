...
$nome_user_get = $_GET['nome_user'];


$consulta = $connect->prepare("SELECT cd.id_user, cd.nomecom_user, cd.nome_user, cd.tipo_user, pu.avatar_user, pu.email_contato, pu.rede_wpp, pu.rede_inst, pu.rede_face FROM cad_user cd INNER JOIN end_user ed ON cd.id_user = ed.id_user INNER JOIN perfil_user pu ON cd.id_user = pu.id_user WHERE cd.nome_user = :nome_user");
$consulta->bindParam(':nome_user', $nome_user_get , PDO::PARAM_STR);
$consulta->execute();
