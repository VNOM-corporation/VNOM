...
$_SESSION['id_get_user'] = $_GET['id_user'];
$id_user = $_SESSION['id_get_user'];


$consulta = $connect->prepare("SELECT cd.nomecom_user, cd.cpf_user, cd.dtnasc_user, cd.sexo_user, cd.nome_user, cd.tipo_user, ed.cep_user, ed.cidade_user, ed.uf_user, ed.rua_user, ed.numcasa_user, ed.bairro_user, ed.comple_user, pf.avatar_user FROM cad_user cd INNER JOIN end_user ed ON cd.id_user = ed.id_user INNER JOIN perfil_user pf ON cd.id_user = pf.id_user WHERE 
cd.id_user = :id_user");
...
if ($tipo_user == 3) {
    $_SESSION['id_get'] = $_GET['id_user'];
    echo"<meta http-equiv='refresh' content='0;url=perfil_edit_prest_adm.php'>"; 
    exit;
}
...
<body>
    ...
    <form action="perfil_update_user_adm.php" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="section-inner">
                        ...
                        <div class="content">
                            ...
                            <div class="item" style="background-color:#f1f1f1; padding: 20px; border-radius: 10px;">
                               <label>Nome Completo</label> <input type="text" placeholder="Nome Completo" class="isd" value="$nomecomp" name="nomecomp"/>
                               <br><br>
                               <input type="hidden" name="id_user" value="$id_user" readonly>
                               <label>CPF</label> <input type="text" placeholder="000.000.000-00"  class="isd" value="$cpf_user" name="cpf_user"/>
                                <br><br>
                                <label>Data de Nascimento</label> <input type="date" placeholder="dd/mm/yyyy" class="isd" value="$dtnasc_user" name="dtnasc_user"/>
                                <br><br>
                                <label>Sexo</label> <input type="text" placeholder="Sexo" class="isd" value="$sexo_user" name="sexo_user"/>
                    </div><!--//section-inner-->                 
                </section><!--//section-->
                HTML;
                ?>
